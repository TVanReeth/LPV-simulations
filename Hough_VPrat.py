...
if __name__ == '__main__':
    ...
    nu = ((abs(kval)+abs(mval))*(abs(kval)+abs(mval) - 1))/float(abs(mval)) + 0.02     # (|k| + |m|) * (|k| + |m| - 1) = 
    gyre_dir = '/lhome/timothyv/Bin/mesa/mesa-12778/gyre/gyre/'
    
    asym = asymptotic(gyre_dir,kval=kval,mval=mval,nmin=1,nmax=201)
    lmbd_est = -asym.lam_fun(nu)
    ...
    np.savetxt(f'/lhome/timothyv/Teaching/sdB_rmodes/r-eigenmode_{kval-4}{mval}.dat', np.array([mu,hr]).T)
    fig = plt.figure(1)
